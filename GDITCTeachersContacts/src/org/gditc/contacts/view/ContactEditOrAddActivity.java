...
public class ContactEditOrAddActivity extends Activity {
	...
	private void setComponentsListener() {
		...
		name.addTextChangedListener(new TextWatcher() {
			...
			public void onTextChanged(CharSequence s, int start, int before, int count) {
				String add_edit_contactName = null;
				add_edit_contactName = name.getText().toString().trim();
				if (add_edit_contactName != null && add_edit_contactName.length() > 1) {
					//add_edit_contactName += "%";
					Cursor cursor = db.searchContactByKeyWords(add_edit_contactName);
					
					if (cursor.moveToNext()) {
						if (state == STATE_INSERT) {
							showToast(cursor.getString(1) + "已存在");
						} else {
							if (cursor.getString(1).equals(add_edit_contactName) && !editContactName.equals(add_edit_contactName)) {
								showToast(cursor.getString(1) + "已存在");
							}
						}
					}
	...
	private void verifyData() {
		...
		if (state == STATE_INSERT) {
			Cursor cursor = db.searchContactByKeyWords(contact_name);
			if (cursor.moveToNext()) {
				showToast(cursor.getString(1) + "已存在！请换个更响亮的名字");
				return;
			}
			cursor.close();
			...
			Cursor cursor = db.searchContactByKeyWords(contact_name);
			if (cursor.moveToNext() && !cursor.getString(1).equals(editContactName)) {
				showToast(cursor.getString(1) + "已存在！请换个更响亮的名字");
				return;
			}
			cursor.close();
